    ...
    def _check_selling_price(self):
        for record in self:
            ...
            if record.selling_price > 0 and record.expected_price > 0:
                # 計算期望售價的 90%
                ninety_percent_expected = record.expected_price * 0.9

                # 使用 float_compare 進行浮點數比較
                # 如果 selling_price < ninety_percent_expected
                if float_compare(record.selling_price, ninety_percent_expected, precision_digits=2) < 0:
